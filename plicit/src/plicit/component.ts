...
export const isAsyncComponent = (x: any): x is AsyncComponent =>
  !!x && isAsyncFunction(x);
...
export const unwrapComponentTree = (
  ...
  const unwrap = (
    ...
    if (isAsyncComponent(component)) {
      return unwrap(
        asyncSignal<any>(async () => await component(attribs), {
          isComputed: true,
          fallback: unwrap(pget(attribs.asyncFallback || PendingSignal)),
        }),
        attribs,
        0,
      );
    ...
    if (isComponent(component)) {
      ...
      withCurrentScope((scope) => {
        if (isLNode(ret)) {
          scope.onMounted.forEach((fun) =>
            ret.addEventListener(ENodeEvent.MOUNTED, () => fun(ret)),
          );
          scope.onBeforeUnmount.forEach((fun) =>
            ret.addEventListener(ENodeEvent.BEFORE_UNMOUNT, () => fun(ret)),
          );
          scope.onUnmounted.forEach((fun) =>
            ret.addEventListener(ENodeEvent.UNMOUNTED, () => fun(ret)),
          );
