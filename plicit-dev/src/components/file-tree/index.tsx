...
const getIcon = (node: FSNode): IconPrimitive => {
  const getSrc = () => {
    ...
    switch (node.type) {
      case EFileType.TEXT:
        return async () => import("../../assets/icons/file-text.svg");
      case EFileType.IMAGE:
        return async () => import("../../assets/icons/file-image.svg");
      case EFileType.PDF:
        return async () => import("../../assets/icons/file-pdf.svg");
      default:
        return async () => import("../../assets/icons/file.svg");
    }
  };
  ...
  const create = (
    ...
    return signal<ITree<FSNode>>({
      ...
      children: (node.children || []).map((child) =>
        create(child, count++, depth + 1),
      ),

      render: (_x) => {
        return (
          <div
            class="select-none text-gray-600 cursor-pointer hover:text-blue-500"
            style={{
              display: "grid",
...
export const FileTree: Component = () => {
  const fsRoot: FSNode = directory({
    ...
    children: [
      ...
      directory({
        name: "Trash",
        children: [file({ name: "old_car.jpeg", type: EFileType.IMAGE })],
      }),
    ],
  });
