import { Component, computedSignal, signal } from "plicit";
import { InputField } from "../input-field";

export const TextReverser: Component = () => {
  const inputValue = signal<string>(() => "");
  const reversedValue = computedSignal(() =>
    Array.from(inputValue.get()).reverse().join(""),
  );

  const inputField = computedSignal(() => (
    <InputField
      placeholder="Enter something"
      value={inputValue.get()}
      onChange={(v) => inputValue.set(() => v + "")}
    />
  ));
